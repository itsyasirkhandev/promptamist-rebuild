...
3. **Frontend Presentation & Forms Layer (Next.js & shadcn/ui)**:
   ...
   - **Settings Panel**: Update `SettingsSection.tsx` to display a Radix UI `Select` component populated with our 6 standard categories, appearing *only* when `isPublic` is toggled ON.
...
- **Contextual UI Form Display**: Displaying the Category select dropdown *only* when the prompt is made public avoids visual noise for draft or template-only prompts that aren't listed on the marketplace.
