...
```
*Note*: Existing `prompts` will need a backfill/mutation to populate `searchableText` (i.e., `${title} ${content} ${tags.join(' ')}`). New and updated prompts will maintain this field automatically upon save.
