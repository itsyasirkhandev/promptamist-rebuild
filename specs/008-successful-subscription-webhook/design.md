...
export const sendProWelcome = internalAction({
  ...
  handler: async (ctx, args) => {
    const sendEmail = Effect.tryPromise({
      try: () => fetch("https://api.brevo.com/v3/smtp/email", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          htmlContent: "<h1>You are now a Pro!</h1><p>Enjoy your newly unlocked entitlements: unlimited prompts, unlimited public sharing, and unlimited static prompt creation.</p>"
        })
      }),
    ...
    const program = Effect.gen(function* () {
      ...
      if (!response.ok) {
         yield* Effect.fail(new Error(`Bravo API Error: ${response.status} ${response.statusText}`));
      }
      return response;
    }).pipe(
