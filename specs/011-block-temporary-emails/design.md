...
- **Clerk Dashboard Native Restriction vs. Code-based Validation**:
  - _Decision_: Enabled native Clerk restrictions.
  - _Rationale_:
    1. **Pre-verification check**: Clerk validates the email _before_ sending a verification code, which prevents wasting outbound email verification credits on spam attempts.
