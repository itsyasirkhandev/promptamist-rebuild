...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // If the user is authenticated, we show the full sidebar-driven dashboard layout
  if (isAuthenticated && !isAuthLoading) {
    // If they are on a public prompt page, they get the public prompt layout but can see they are authenticated
  ...
  // Otherwise (unauthenticated or auth is still loading), we show the premium public layout
  return (
    <div className="flex min-h-screen w-full flex-col bg-neutral-50 transition-colors duration-500 dark:bg-stone-950">
      <LandingHeader />
      <main className="flex-1">{children}</main>
