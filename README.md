...
flowchart TD
    ...
    subgraph Backend ["Convex Serverless Backend (Strict 3-Tier)"]
        API["1. Authenticated API Boundary<br>(convex/authed/helpers.ts)<br>• convex-helpers wrappers<br>• authedQuery / authedMutation"]

        Logic["2. Functional Logic Engine<br>(convex/authed/*.ts)<br>• Effect TS (Effect.gen pipelines)<br>• Typed error handling<br>• Action orchestration"]

        DAL["3. Data Access Layer (DAL)<br>(convex/dal/*.dal.ts)<br>• Direct ctx.db calls isolated here"]

        DTO["4. DTO Mapping Layer<br>(convex/dto.ts)<br>• Output sanitization"]
...
flowchart TD
    subgraph UserSync ["Cycle 1: User Registration & Synchronization"]
        ...
        C1 -->|HTTP POST webhook payload| Webhook["Convex HTTP Router<br>(convex/http.ts /clerk-users-webhook)"]
        Webhook -->|1. Verify with Svix<br>2. Ingest via Effect pipeline| Upsert["upsertFromClerk<br>(convex/users.ts)"]
        Upsert -->|3. Insert user info| DAL_User["users.dal.ts"]
        DAL_User -->|4. Save| ConvexDB[(Convex DB)]

        %% Asynchronous Scheduler Flows
        Upsert -->|5a. Schedule background task| EmailAction["internal.emails.sendWelcomeEmail<br>(convex/emails.ts)"]
        EmailAction -->|Send SMTP Payload| Brevo["Brevo SMTP REST API"]
        Brevo -->|Deliver to User Inbox| WelcomeEmail["Welcome Email Received"]

        Upsert -->|5b. Schedule background task| PolarAction["internal.private.polar.createPolarCustomerBackground<br>(convex/private/polar.ts)"]
        ...
        PolarAction -->|Save customer ID| SavePolarId["savePolarCustomerIdInternal<br>(convex/users.ts)"]
        SavePolarId -->|Write field| ConvexDB
    end

    subgraph SubBilling ["Cycle 2: Subscription & Entitlement Lifecycle"]
        P1["User clicks 'Upgrade' in UI"] -->|Call Next.js Server Action| SA["createCheckoutSession<br>(src/app/actions/polar.ts)"]
        SA -->|1. Fetch polarCustomerId| QueryCustomer["getPolarCustomerId<br>(convex/authed/users)"]
        ...
        PolarPlatform2 -->|HTTP POST webhook payload| PolarWebhook["Convex HTTP Router<br>(convex/http.ts /polar-webhook)"]
        PolarWebhook -->|1. Base64 encode secret<br>2. Verify with Svix| PolarWebhook

        PolarWebhook -->|3. If tier upgraded to Pro| ProEmail["internal.private.emails.sendProWelcome<br>(convex/private/emails.ts)"]
        ProEmail -->|Send SMTP Payload via Effect| Brevo
        Brevo -->|Deliver to User Inbox| ProWelcomeEmail["Pro Celebration Email"]

        PolarWebhook -->|4. Ingest subscription status| UpdateSub["updateSubscriptionTier<br>(convex/users.ts)"]
